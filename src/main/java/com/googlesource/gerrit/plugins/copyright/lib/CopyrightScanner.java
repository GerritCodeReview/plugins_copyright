 ...
 *
 * <p>In general, configure the first-party (1p) and forbidden owners, and any generic owner
 * matches get classified as third-party (3p) automatically. Generally, only configure
 * third-party (3p) owners that the generic pattern will not match for some reason.
 *
...
public final class CopyrightScanner {
   ...
   */
  public ImmutableList<Match> findMatches(
      String name, long size, IndexedLineReader source) throws IOException {
    ...
    while (matcher.find()) {
      ...
      if (numLicenses == 0 && numBuilt == 0 && numUnknown <= MATCH_THRESHOLD) {
        ...
        if (isForbiddenLicense(license)) {
          ...
          builder.add(
              new Match(
                  PartyType.THIRD_PARTY, license, startLine, endLine, mr.start(), mr.end()));
          numBuilt++;
          continue;
        } else if (isFirstPartyOwner(owner)) {
          builder.add(
              new Match(
                  PartyType.FIRST_PARTY, license, startLine, endLine, mr.start(), mr.end()));
          numBuilt++;
          continue;
        }
   ...
   * of the line so it does not pick up spurious additional text. The generic owner pattern does
   * not end in a specific word so it often includes spurious additional words like #ifdef or
   * #ifndef, which interfere when comparing the match against known author/owner patterns. This
   * method strips the most common non-author/owner words from the end of the match.
   */
