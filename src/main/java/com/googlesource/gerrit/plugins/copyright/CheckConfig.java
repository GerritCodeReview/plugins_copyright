...
import com.googlesource.gerrit.plugins.copyright.CopyrightReviewApi;
 ...
 *
 * {@code main} implements a command-line tool for measuring performance against a large test file
 * constructed to trigger excessive backtracking.
 */
public class CheckConfig {
   ...
   *
   * Uses {@code reviewApi}, when given, to verify account information. When {@code pluginEnabled}
   * is false, treats errors as warnings.
   */
  public static void checkProjectConfig(
    ...
    CurrentUser fromUser =
        reviewApi == null
            ? null : reviewApi.getSendingUser(trialConfig.scannerConfig.fromAccountId);
    if (Strings.nullToEmpty(trialConfig.scannerConfig.reviewLabel).trim().isEmpty()) {
              ...
              trialConfig.scannerConfig.pluginKeyRequired(
                  ...
                  "please use \""
                      ...
                      + (trialConfig.scannerConfig.fromAccountId < 1 || fromUser == null
                          ? "the plugin" : fromUser.getLoggableName())
                      + " will vote on"),
      ...
      String[] voters = trialConfig.configProject.getStringList(
      ...
      voters = trialConfig.configProject.getStringList(
    ...
    if (trialConfig.scannerConfig.reviewers.isEmpty()) {
      trialConfig.scannerConfig.messages.add(
          ScannerConfig.warningMessage(
              trialConfig.scannerConfig.pluginKeyRequired(
                  ScannerConfig.KEY_REVIEWER,
                  "adding no qualified reviewer may cause confusion")));
    }
    ...
    //       a la https://gerrit.googlesource.com/gerrit/+/refs/heads/master/java/com/google/gerrit/server/restapi/change/PostReview.java#265
   ...
   *
   * When a new commit alters the configured scanner patterns, the push will fail with a message
   * to download the plugin source, to run a shell script that runs {@code main} below, and to
   * copy the output on success into the commit message.
   *
   * This method scans the commit message to find the copied text. If the text was created for the
   * same pattern signagure, this method returns a single valid finding with the number of
   * microseconds it took to scan a large file, which can be used to block patterns that cause
   * excessive backtracking.
   *
   * If the commit message contains one or more copied texts for other pattern signatures, this
   * method retuns an invalid finding for each.
   *
   * If the commit message contains no copied texts, this method returns an empty list of findings,
   * which {@link com.googlesource.gerrit.plugins.copyright.CopyrightConfig} uses as a signal to
   * instruct the submitter to run the shell script in the first place.
   *
   ...
   *
   * Problems include finding no time signature, finding a time signature for a different commit, or
   * finding a valid time signature for a test that took longer than {@code maxElapsedSeconds}.
   */
  ...
  private IndexedLineReader largeFile() {
    ...
    for (int j = 0; j < 2; j++) {  // 16k + 48k = 64k, x 2 = 128k, x 2 = 256k -- max search size
      for (int i = 0; i < 16; i++) {
        sb.append(onek);
      }
      for (int i = 0; i < 48; i++) {
  ...
  public static void main(String[] args) {
    ...
    try {
      IndexedLineReader ilr = new IndexedLineReader(
