...
public class CopyrightReviewApi {
   ...
   *
   * If no output found in the commit message, directs the submitter to run the tool and copy its
   * output into the commit message.
   *
   * If the output was found for a different scanner pattern, directs the submitter to run again for
   * the current commit.
   *
   * Otherwise, the duration of the timing run must exceed the configured limit. Describes patterns
   * known to cause problems and directs the submitter to change the pattern.
   *
  ...
  private ImmutableList<CommentInput> getCommitMessageComments(
    ...
    if (findings.isEmpty()) {
      sb.append(
          "While most patterns are fine, some patterns can force your gerrit server\n");
      sb.append(
          "to work too hard. To protect your server, there is a tool that can\n");
      sb.append(
          "detect these patterns before the configuration gets submitted.\n\n");
      sb.append(
          "Please use git to download the tool from:\n");
      sb.append(
          "https://gerrit.googlesource.com/plugins/copyright/+/refs/heads/master\n\n");
      sb.append(
          "After downloading, run tools/check_new_config.sh (requires bazel):\n");
      sb.append(
          "<path>/tools/check_new_config.sh '");
      sb.append(pluginName);
      sb.append("' '<path>/project.config'\n");
      sb.append(
          "and copy it's output to your commit message.\n\n");
      sb.append(
          "e.g. if your local All-Projects is at workspace/All-Projects and if you\n");
      sb.append(
          "downloaded plugins/copyright to workspace/copyright, you might run:\n");
      sb.append(
          "../copyright/tools/check_new_config.sh '");
      sb.append(pluginName);
      sb.append("' project.config\n");
      sb.append(
          "from the workspace/All-Projects directory.\n");
      ci.line = 0;
      ci.unresolved = true;
      ...
      sb.append(
          "The result suggests a pattern that causes excessive backtracking.\n");
      ...
      sb.append("Wildcards:\n");
      sb.append(
          "  The scanner automatically handles .* and .+ patterns, but it's");
      sb.append(
          "  possible to accidentally compose equivalents or near-equivalents:");
      sb.append(
          "  e.g. (?:[a][^a])* or [\\p{N}\\p{L}\\p{P}]+ match nearly everything\n");
      sb.append(
          "  If your new pattern contains something similar, consider using .* for\n");
      sb.append(
          "  automatic handling instead, or use match a smaller character class.\n\n");
      sb.append("Unbounded repetitions:\n");
      sb.append(
          "  If your new pattern uses * or + for unlimited repetitions, consider\n");
      sb.append(
          "  using a more limited repetition like {0,10} or {1,50} that is long\n");
      sb.append(
          "  enough to match what you need but short enough to scan quickly.\n\n");
      sb.append("Zero-length look-ahead or look-behind:\n");
      sb.append(
          "  Patterns like (?!word), (?=word), (?<!word). (?<=word) etc. can cause\n");
      sb.append(
          "  excessive backtracking too. Sometimes, it is faster to match a little\n");
      sb.append(
          "  more than needed and use an excludePattern to eliminate unwanted hits.\n");
      sb.append(
          "  e.g. forbiddenPattern = owner some pattern \\p{L}*\n");
      sb.append(
          "       excludePattern = some pattern word\n\n");
      sb.append(
   ...
   *
   * Each finding identifies the position in the commit message, the validity for the current
