...
public class CopyrightScannerTest {
  ...
  private static boolean tsanRunning = false;

  @BeforeClass
  public static void setUpClass() {
    // Check if we are running with TSAN enabled, exit timeout will be longer then.
    // Copied shamelessly from //java/com/google/testing/junit/runner/GoogleTestRunner.java
    tsanRunning = false;
    try {
      HotSpotDiagnosticMXBean hsb =
          ManagementFactory.getPlatformMXBean(HotSpotDiagnosticMXBean.class);
      if (hsb != null && Boolean.parseBoolean(hsb.getVMOption("GoogleTsan").getValue())) {
        tsanRunning = true;
      }
    } catch (IllegalArgumentException e) {
      // No such flag, same as false.
    }
  }
  ...
  public void tesFindMatch_largeFileNoMatches() throws Exception {
    if (tsanRunning) {  // tsan is too much slower
      return;
    }
