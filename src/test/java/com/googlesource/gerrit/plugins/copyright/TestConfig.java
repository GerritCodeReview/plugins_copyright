...
class TestConfig {
  ...
  TestConfig(
      ...
      String pluginName,
      TestAccount owner,
      TestRepository<InMemoryRepository> testRepo) throws Exception {
  ...
  static final Consumer<PluginConfig> BASE_CONFIG = pCfg -> {
      pCfg.setString(KEY_REVIEW_LABEL, "Copyright-Review");
      pCfg.setStringList(KEY_EXCLUDE, ImmutableList.of("EXAMPLES"));
      pCfg.setStringList(
          KEY_FIRST_PARTY,
          ImmutableList.of("APACHE2", "ANDROID", "GOOGLE", "EXAMPLES"));
      pCfg.setStringList(
          KEY_THIRD_PARTY,
          ImmutableList.of("BSD", "MIT", "EPL", "GPL2", "GPL3", "PSFL"));
      pCfg.setStringList(
          KEY_FORBIDDEN,
          ImmutableList.of(
              "AGPL",
  ...
  static final Consumer<PluginConfig> ENABLE_CONFIG = pCfg -> {
       pCfg.setBoolean(KEY_ENABLE, true);
    };
  static final Consumer<PluginConfig> DISABLE_CONFIG = pCfg -> {
  ...
  /**
   * Make a copy of an existing label configuration {@code fromLabel} under a different name,
   * {@code toLabel}.
   *
   * Useful shortcut for configuring a Copyright-Review label based on the default Code-Review
   * label.
   */
  ...
  PushOneCommit.Result push(PushOneCommit.Factory pushFactory) throws Exception {
    ...
    PushOneCommit pushCommit = pushFactory.create(
        author,
        testRepo,
        subject,
  ...
  void setVoters(String ref, String label, Voter... voters) throws Exception {
    ...
    for (Voter v : voters) {
      builder.add(
          vString(v.minVote) + ".." + vString(v.maxVote) + " group " + v.groupName);
    }
  ...
  /**
   * Replaces the plugin section in {@code this.configProject} with the contents of
   * {@code this.configPlugin}.
   */
